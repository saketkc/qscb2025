...
We will take few patients: H07, H14, H06, H16, H23
...
```{r}
# this is log(xij/Cj+1)
layer.data <- LayerData(object = object.subset, layer = "data", assay = "RNA")

# this is exp(log(xij/Cj+1)) = xij/Cj+1
layer.data.remove.log <- as(exp(layer.data), "dgCMatrix")

df <- data.frame(mean = rowMeans(layer.data), variance = rowVars(layer.data), variance_nonlogged = rowVars(layer.data.remove.log))

ggplot(df) + 
  geom_point(mapping = aes(mean, variance, color='log')) +
  geom_point(mapping = aes(mean, variance_nonlogged, color='nologged')) +
geom_abline() +   theme_bw(base_size = 14)  + scale_y_log10()

```

So the mean variance has now "stabilized". , however you will notice that the variance stabilization is not uniform - there is a bend in the curve at the beginning.
...
```{r}
data.to.plot <- FetchData(object = object.subset, layer = "counts", vars = c( "MALAT1", "RPL10", "ALB", "MS4A1", "CD2", "nCount_RNA"))
head(data.to.plot)
```
...
```{r}
FeaturePlot(integrated, features = "ALB", reduction = "umap.harmony")
```
